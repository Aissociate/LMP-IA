/*
  # Fix Market Sentinel™ System Tables

  1. Modifications
    - Ajouter colonnes IA à search_alerts (pas market_search_alerts)
    - Créer table market_relevance_scores avec les bonnes références
    - Créer table market_sentinel_stats
...
-- Ajouter des colonnes pour le scoring IA à search_alerts
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'search_alerts' AND column_name = 'enable_ai_scoring'
  ) THEN
...
CREATE TABLE IF NOT EXISTS market_relevance_scores (
  ...
  alert_id uuid REFERENCES search_alerts(id) ON DELETE CASCADE,
  ...
  score_category text NOT NULL DEFAULT 'no_go' CHECK (score_category IN ('go', 'conditional', 'no_go')),

  -- Recommandation IA
  ai_recommendation text NOT NULL DEFAULT 'ignore' CHECK (ai_recommendation IN ('respond', 'ignore', 'request_expert', 'order_memory')),
  ai_reasoning text,
  ...
  user_action text CHECK (user_action IN ('respond', 'ignore', 'request_expert', 'order_memory')),
